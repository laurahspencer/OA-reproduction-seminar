...
(gg.both <- ggplot() + 
  ...
  theme(legend.position = "none") +
  #guides(colour = guide_legend(override.aes = list(size=4))) + 
  geom_label(aes(x=-1.56, y=0.5), label=paste("Model with full pH range: χ2 p-value =",
                       round(Anova(best)[,"Pr(>Chisq)"], digits =5), sep=" "),
             colour="gray15", size=3, alpha=0.3, fill="#fff2ae") +
   ...
   geom_label(aes(x=-1.2, y=0.84,
            ...
            colour="gray15", alpha=0.1, fill="#2ca25f", size=3))
...
```{r}
all.plots <- ggarrange(gg.both + ylab(NULL) + xlab(NULL) + theme(legend.position="none"), 
...
tiff(file = "fertilization-by-phylum-20210308.tiff", width=2200, height=3700, res=250)
